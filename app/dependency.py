...
def get_current_user(
    credentials: str = Depends(oauth2_scheme), db: Session = Depends(get_db)
) -> User:
    ...
    """
    try:
        token = jwt.decode(credentials, settings.SECRET_KEY, [settings.ALGORITHM])
...
def validate_internal_secret(x_internal_secret: Annotated[str | None, Header()] = None) -> None:
    ...
    """
    if x_internal_secret is None or not compare_digest(
        x_internal_secret, settings.INTERNAL_BOT_SECRET
    ):
